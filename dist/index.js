...
const container_1 = require("./user/shared/container");
const container_2 = require("./reserva/shared/container");
async function main() {
    console.log("🚀 Sistema de Gestión de Usuarios y Reservas");
    console.log("============================================\n");
    // Contenedor de usuarios
    const userContainer = container_1.UserContainer.getInstance();
    const usuarioController = userContainer.getUsuarioController();
    // Contenedor de reservas
    const reservaContainer = container_2.ReservaContainer.getInstance();
    ...
    try {
        ...
        todasLasReservas.slice(0, 3).forEach((reserva, index) => {
            console.log(`${index + 1}. Reserva ${reserva.id.slice(-8)} - ${reserva.numeroPersonas} personas - ${reserva.estado} (${reserva.fechaHora.toLocaleDateString()})`);
        });
        console.log("\nReservas activas:");
        ...
        reservasActivas.slice(0, 3).forEach((reserva, index) => {
            console.log(`${index + 1}. Reserva ${reserva.id.slice(-8)} - ${reserva.numeroPersonas} personas (${reserva.fechaHora.toLocaleDateString()})`);
        });
        ...
        if (reservaEspecifica) {
            console.log(`Reserva encontrada: ${reservaEspecifica.numeroPersonas} personas para el ${reservaEspecifica.fechaHora.toLocaleDateString()} - Estado: ${reservaEspecifica.estado}`);
        }
        console.log();
        ...
        reservasFinales.slice(0, 5).forEach((reserva, index) => {
            console.log(`${index + 1}. ${reserva.numeroPersonas} personas - ${reserva.estado} (${reserva.fechaHora.toLocaleDateString()})`);
        });
    }
    catch (error) {
